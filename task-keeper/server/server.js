...
server.post("/api/v1/task", (req, res) => {
  ...
  const query =
    "insert into task(Content, Duedate, Statustask, Assigned) values (?,?,?,?)";
...
server.put("/api/v1/task/:id", (req, res) => {
  ...
  const query =
    "UPDATE task SET Content=?, Duedate=?, Statustask=?, Assigned=? WHERE TaskId=?";
  ...
  database.query(query, values, (err, result) => {
    ...
    return res.status(200).json({
      status: "success",
      message: "Cập nhật task thành côn",
      data: result,
    });
  });
});
